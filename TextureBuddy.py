...
class TB_PT_TextureBuddyPanel(bpy.types.Panel):
...
class TB_OT_TextureBuddyUV(bpy.types.Operator):
                ...
                if f.select:
                    ...
                    if len(obj.data.materials) > matIndex:
                        if obj.data.materials[matIndex] is not None:
                            obMat = context.active_object.data.materials[matIndex]
                            paintIdx = obMat.paint_active_slot
                            if len(obMat.texture_paint_images) > paintIdx:
                                texture = obMat.texture_paint_images[paintIdx]
                                try:
                                    textureWidth = texture.image.size[0]
                                    textureHeight = texture.image.size[1]
                                except:
                                    pass
                                nX = f.normal.x
                                nY = f.normal.y
...
class TB_OT_TextureBuddyPin(bpy.types.Operator):
...
class TB_OT_TextureBuddyNudge(bpy.types.Operator):
...
def register():
    bpy.utils.register_class(TB_PT_TextureBuddyPanel)
    bpy.utils.register_class(TB_OT_TextureBuddyUV)
    bpy.utils.register_class(TB_OT_TextureBuddyNudge)
    bpy.utils.register_class(TB_OT_TextureBuddyPin)
...
def unregister():
    bpy.utils.unregister_class(TB_PT_TextureBuddyPanel)
    bpy.utils.unregister_class(TB_OT_TextureBuddyUV)
    bpy.utils.unregister_class(TB_OT_TextureBuddyNudge)
    bpy.utils.unregister_class(TB_OT_TextureBuddyPin)
