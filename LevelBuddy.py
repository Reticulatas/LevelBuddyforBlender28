...
def create_new_boolean_object(scn, name):
    ...
    #ob.layers[scn.collection] = True
    if ob.name not in scn.collection.objects:
        scn.collection.objects.link(ob)
    scn.view_layers[0].update()
    scn.view_layers[0].objects.active = ob
    ob.select_set(True, view_layer=scn.view_layers[0])
    return ob
...
def get_active_layers():
    ...
    active_layer[scn.collection] = True
    return active_layer
...
class LB_PT_LevelBuddyPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.prop_search(scn, "remove_texture", bpy.data, "materials", icon="MOD_EXPLODE")
        ...
        col.prop_search(scn, "brush_material", bpy.data, "materials", icon="MOD_EXPLODE", text="")
        layout.separator()
        col = layout.column(align=True)
...
class LB_OT_LevelNewSector(bpy.types.Operator):
...
class LB_OT_LevelNewBrush(bpy.types.Operator):
...
class LB_OT_LevelUpdateSector(bpy.types.Operator):
...
class LB_OT_LevelBuddyCopy(bpy.types.Operator):
...
class LB_OT_LevelCleanupPrecision(bpy.types.Operator):
...
class LB_OT_LevelEmptyTrash(bpy.types.Operator):
...
class LB_OT_LevelBuddyBuildMap(bpy.types.Operator):
        ...
        if self.bool_op == 'EXPORT':
            ...
            for x in sector_list:
                if x != sector_list[0]:
                    ...
                    level_map.data = bpy.data.meshes.new_from_object(ob.evaluated_get(depsgraph), preserve_all_data_layers=True, depsgraph=depsgraph)
...
def register():
    bpy.utils.register_class(LB_PT_LevelBuddyPanel)
    bpy.utils.register_class(LB_OT_LevelBuddyBuildMap)
    bpy.utils.register_class(LB_OT_LevelNewSector)
    bpy.utils.register_class(LB_OT_LevelBuddyCopy)
    bpy.utils.register_class(LB_OT_LevelUpdateSector)
    bpy.utils.register_class(LB_OT_LevelNewBrush)
    bpy.utils.register_class(LB_OT_LevelCleanupPrecision)
    bpy.utils.register_class(LB_OT_LevelEmptyTrash)


def unregister():
    bpy.utils.unregister_class(LB_PT_LevelBuddyPanel)
    bpy.utils.unregister_class(LB_OT_LevelBuddyBuildMap)
    bpy.utils.unregister_class(LB_OT_LevelNewSector)
    bpy.utils.unregister_class(LB_OT_LevelBuddyCopy)
    bpy.utils.unregister_class(LB_OT_LevelUpdateSector)
    bpy.utils.unregister_class(LB_OT_LevelNewBrush)
    bpy.utils.unregister_class(LB_OT_LevelCleanupPrecision)
    bpy.utils.unregister_class(LB_OT_LevelEmptyTrash)
